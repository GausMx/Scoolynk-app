...
      const result = await BrowserOCR.extractScores(
        scanImage,
        (progress) => {
...
        setError('No scores found. Please ensure the image is clear.');
        return;
      }

      setSubjects(result.scores);
      alert(`✅ Found ${result.scores.length} subject(s)!\nConfidence: ${result.confidence.toFixed(1)}%\n\nPlease review the scores.`);
...
                        <li>Runs in your browser using Tesseract.js</li>
...
            <strong>Tip:</strong> Fill the form on the right based on the template.
          </div>
        </div>
      </div>
...
              {subjects.map((subject, index) => {
                const total = (Number(subject.ca1) || 0) + (Number(subject.ca2) || 0) + (Number(subject.exam) || 0);
                const percent = total; // Total is out of 100
                let grade = 'F';
...
            rows="3"
            value={comments.principal}
            disabled={true} // Principal's comment should be read-only for the teacher
            placeholder="Principal's comment will appear here after approval."
          ></textarea>
        </div>
      </div>
      <datalist id="subject-suggestions">
