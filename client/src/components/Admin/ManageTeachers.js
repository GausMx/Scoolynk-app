...
// --- Icons ---
const IconTeachers = ({ className = "" }) => <i className={`bi bi-person-workspace fs-4 ${className}`}></i>;
    ...
    const handleAddOrEdit = (formData) => {
        ...
        setTimeout(() => {
            if (modalState.mode === 'add') {
                const newTeacher = { ...formData, id: 't' + Date.now() };
                setTeachers((prev) => [...prev, newTeacher]);
                setMessage(`Teacher '${newTeacher.name}' added successfully.`);
            } else {
                setTeachers((prev) =>
            ...
            <div className="card shadow-lg rounded-4 p-4 mb-4 border-0">
                ...
                <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 border-bottom pb-3">
                    ...
                    <button
                        ...
                        disabled={loading}
                    >
                        <IconPlus className="fs-5 me-2" /> Add New Teacher
                    </button>
                </div>
                                    ...
                                    <tr key={t.id}>
                                        ...
                                        <td className="d-none d-lg-table-cell">
                                            {t.assignedClasses.map((cls) => (
                                                <span
                                                    key={cls}
                                                    className="badge bg-secondary me-1 text-truncate"
                                                    style={{ maxWidth: '80px' }}
                                                >
                                                    {cls}
                                                </span>
                                        ...
                                        <td className="text-center">
                                            <button className="btn btn-sm btn-outline-secondary me-2 rounded-3" onClick={() => setModalState({ isOpen: true, mode: 'edit', currentTeacher: t })} disabled={loading}>
                                                <IconEdit className="fs-6" />
                                            </button>
                                            <button className="btn btn-sm btn-outline-danger rounded-3" onClick={() => handleDelete(t.id, t.name)} disabled={loading}>
                            ...
                            <div className="modal-content rounded-4 shadow-lg border-0">
                                <div className="modal-header bg-light border-bottom">
                                    <h5 className="modal-title fw-bold">
                                        {modalState.mode === 'add' ? 'Add New Teacher' : 'Edit Teacher'}
                                    </h5>
                                    <button
                                        type="button"
                                        className="btn-close"
                                        onClick={() => setModalState({ isOpen: false, mode: 'add', currentTeacher: null })}
                                    ></button>
                                </div>
